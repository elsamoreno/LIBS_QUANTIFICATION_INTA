...
def plot_and_save_spectra(wavelengths, espectros, titulo, archivo_salida, nombres=None, 
    ...
    # --- Prepare labels ---
    use_labels = nombres is not None
    if use_labels:
        if len(nombres) != len(espectros):
            raise ValueError(f"Length of labels ({len(nombres)}) != number of spectra ({len(espectros)})")
    else:
        nombres = [None] * len(espectros)  # Placeholder for zip compatibility
    ...
    for wl_entry, sp_entry, lbl in zip(wavelengths, espectros, nombres):
        if isinstance(wl_entry, np.ndarray) and wl_entry.ndim == 1:
            ax.plot(wl_entry, sp_entry, label=lbl if use_labels else None)
        else:
            wl_blocks = [np.asarray(b) for b in wl_entry]
            sp_blocks = [np.asarray(b) for b in sp_entry]
            ...
            ax.plot(np.array(merged_wl), np.array(merged_sp), label=lbl if use_labels else None)
